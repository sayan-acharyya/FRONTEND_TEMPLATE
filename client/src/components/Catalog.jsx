...
const Catalog = () => {
  ...
  const { returnBookPopup, readBookPopup } = useSelector(state => state.popup);
  ...
  const booksToDiaplay = filter === "borrowed" ? borrowedBooks : overDueBooks;
  ...
  return <>
    ...
    <main className="relative flex-1 p-6 pt-28 ">
      ...
      <header className="flex flex-col gap-3 sm:flex-row md:items-center">
        <button className={`relative rounded sm:rounded-tl-none sm:rounded-br-none sm:rounded-tr-lg sm:rounded-bl-lg
          text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter === "borrowed" ? "bg-black text-white border-black " : "bg-gray-200 text-black border-gray-200 hover:bg-gray-300 "}`}
          onClick={() => setFilter("borrowed")}
        >
          Borrowed Books
        </button>
        <button className={`relative rounded sm:rounded-tl-none sm:rounded-br-none sm:rounded-tr-lg sm:rounded-bl-lg
          text-center border-2 font-semibold py-2 w-full sm:w-72 ${filter === "overdue" ? "bg-black text-white border-black " : "bg-gray-200 text-black border-gray-200 hover:bg-gray-300 "}`}
      ...
      {
        booksToDiaplay && booksToDiaplay.length > 0 ? (
