...
const BookManagement = () => {
  ...
  const sesrchedBooks = books.filter((book) => (
  ...
  return <>
    ...
    <main className="relative flex-1 p-6 pt-28 ">
      ...
      <header className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center">
        <h2 className="text-xl font-semibold md:text-2xl md:font-semibold">
        ...
        <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4">
          {
            isAuthenticated && user?.role === "Admin" && (
              <button
                className="relative pl-14 w-full sm:w-52 flex gap-4 justify-center items-center py-2 px-4 bg-black 
              text-white rounded-md hover:bg-gray-800"
                ...
                text-black w-[25px] h-[25px] text-[27px] absolute left-5">+</span>
                Add Book
              </button>
            )
          }
